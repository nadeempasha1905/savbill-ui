...
cb_app.controller('cb_controller',function($scope,$rootScope,$location,$route,$timeout,load_ui,remote,cookie,$window){
	...
	$rootScope.$on('$locationChangeSuccess',function(){
		 ...
		 console.log('form----> ',$rootScope.breadcrumb[$rootScope.breadcrumb.length-1]);
		 $timeout(function(){
			 $rootScope.getFormPrivileges($rootScope.breadcrumb[$rootScope.breadcrumb.length-1]);
		 },2000);
	});
	...
	$rootScope.getFormPrivileges = function(form_id){
		
		$rootScope.sav_menus.map(function(e,index){
			
			if(form_id === e.form_id){
				$rootScope.current_form_privilege = e.form_privileges;
				$rootScope.current_form_select = ((e.form_privileges).substring(0,1) === '1' ? true : false);
				$rootScope.current_form_insert = ((e.form_privileges).substring(1,2) === '1' ? true : false);
				$rootScope.current_form_update = ((e.form_privileges).substring(2,3) === '1' ? true : false);
				$rootScope.current_form_delete = ((e.form_privileges).substring(3,4) === '1' ? true : false);
				
				console.log("$rootScope.current_form_privilege",$rootScope.current_form_privilege);
				console.log("$rootScope.current_form_select",$rootScope.current_form_select);
				console.log("$rootScope.current_form_insert",$rootScope.current_form_insert);
				console.log("$rootScope.current_form_update",$rootScope.current_form_update);
				console.log("$rootScope.current_form_delete",$rootScope.current_form_delete);
			}else{
				$rootScope.current_form_privilege = null;
				$rootScope.current_form_select = false;
				$rootScope.current_form_insert = false;
				$rootScope.current_form_update = false;
				$rootScope.current_form_delete = false;
			}
